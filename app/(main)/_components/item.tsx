...
export const Item = ({
  ...
  const onCreate=(
    ...
    event.stopPropagation();
    if(!id)return;
    const promise =create({title:"New Titled",parentDocument:id}).then((documentId)=>{
      if(!expanded){
        onExpand?.();
      }
    ...
    >
      ...
      <span className="truncate">{label}</span>
			{isSearch&&(
				...
				h-5 select-none items-center gap-1 rounded border bg-muted px-1.5 font-mono text-[10px] font-medium text-muted-foreground
				 opacity-100">
